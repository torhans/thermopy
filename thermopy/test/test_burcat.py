...
def match_elements(compound,elements):
    ret=True
    for celem,telem in zip(compound._elements,elements):
        ret &= str(celem[0]).upper()==str(telem[0]).upper()
        ret &= celem[1]==telem[1]
    if not ret:
        print(compound._elements,elements)
    return ret
...
def test_reactions_test():
    ...
    N2=database.set_compound("N2 REF ELEMENT")
    O2=database.set_compound("O2 REF ELEMENT")
    ...
    reaction = burcat.Reaction(None,1000, reacts, prods, reacts_coefs,prods_coefs)
